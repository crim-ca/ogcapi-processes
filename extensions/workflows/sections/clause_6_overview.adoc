...
* extending execution requests submitted to `/processes/{processID}/execution`
   ** referencing local and remote nested processes as inputs (`"process"`) (<<clause_nested-processes,Section 7>>),
   ** referencing local and remote OGC API collections as inputs (`"collection"`) (<<section_collection_input,Section 8>>),
   ** modifying data accessed as inputs and returned as outputs (filtering with `"filter"`, selecting and deriving fields with `"properties"`, sorting with `"sortBy"`) (<<clause_fields-modifiers,Section 9>>),
   ** deploying workflows defined using such execution request as processes using https://github.com/opengeospatial/ogcapi-processes/tree/master/extensions/deploy_replace_undeploy/standard[_OGC API - Processes - Part 2: Deploy, Replace, Undeploy_], with the ability to paremeterize inputs (`"$input"`) and select outputs (`"$output"`) (<<clause_deployable-workflows,Section 10>>),
...
* ad-hoc execution of workflows defined using the Common Workflow Language (<<clause_cwl-workflows,Section 12>>), and
* ad-hoc execution of workflows defined using the OpenEO Process Graphs (<<clause_openeo-workflows,Section 13>>).

[[sc_architecture-diagram]]
...
[[sc_obstacles-addressed]]
==== Obstacles addressed

[[sc_long-batch-processing-times]]
...
[[sc_integrate-diverse-data-and-processes]]
...
[[sc_collection-input-output-considerations]]
...
data transfer APIs, area, time and resolution of interest, and selection of outputs, therefore greatly enhancing the re-usability of the workflows.
This negotiation happens at each hop along a workflow chain, and the end-user client need only decide on these for the server it directly connects to (immediate hop).
