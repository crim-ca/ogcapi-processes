...
* extending execution requests submitted to `/processes/{processID}/execution`
   ...
   ** deploying workflows defined using such execution request as processes using https://github.com/opengeospatial/ogcapi-processes/tree/master/extensions/deploy_replace_undeploy/standard[_OGC API - Processes - Part 2: Deploy, Replace, Undeploy_], with the ability to parametrize inputs (`"$input"`) and select outputs (`"$output"`) (<<sc_deployable-workflows,Section 10>>),
