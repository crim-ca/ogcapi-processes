...
part:: The Implementation SHALL support selecting specific outputs of processes involved in workflow definition using the `"$output"` key and a corresponding output identifier value as part of an output of a process (within the `outputs` of an execution request or nested input process).
