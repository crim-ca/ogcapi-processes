...
[[sc_core-landingpage_operation]]
==== Operation

include::../requirements/core/REQ_landingpage-op.adoc[]


[[sc_core-landingpage_response]]
==== Response
...
[[sc_core-landingpage_error-situations]]
==== Error situations
...
[[sc_core-api-definition_operation]]
==== Operation
...
[[sc_core-api-definition_response]]
==== Response
...
[[sc_core-api-definition_error-situations]]
==== Error situations
...
[[sc_core-conformance_error-situations]]
==== Error situations
...
In this second example, the property `geometryInput` has a cardinality of greater than 1 and value instances can be one of a number of enumerated media types.  The schema of `geometryInput` from the <<sc_process-description,OGC process description>> for the process might be:
...
[%unnumbered]
====
Consider a process input named `complexObjectInput` with the following definition from an <<sc_process-description,OGC process description>>:
...
For the purposes of validation, the server needs to only validate the component of the <<qualified-value,qualified value>> that is an instance of <<input-value-schema,value.yaml>> against the `schema` fragment from the <<sc_process-description,OGC process description>>.  Specifically, the validation target is:
...
NOTE: This example makes use of an <<sc_process-description,OGC process description>>.  However, any other process description vocabulary may be used and applied, for the purpose of validation, in a similar manner.
====
