...
3. Verify that the `links` section of each process summary of the response contains a link with `rel=alternate` for each response representation the service claims to support in its <<clause_conformance,conformance document>>.
--
====
